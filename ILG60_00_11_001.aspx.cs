...
{
    public partial class ILG60_00_11_001 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            //if (!IsPostBack)
            {
                string filename = "ILG60-00-11-001";
                string para = "EvidenceInID";
                try
                {
                    ...
                    {
                        //var js = new ILG60_00_11_001Json();
                        var js = JsonConvert.DeserializeObject<ILG60_00_11_001Json>(requestFromPost);
                        //js.EvidenceInID = 501;
                        if (js != null)
                        {
                           
                            if (js.EvidenceInID > 0)
                            {
                                ReportDocument rpt = new ReportDocument();                                
                                rpt.Load(Server.MapPath("~/ReportModel/" + filename + ".rpt"));
                                rpt = new cryslogin().Getsublogin(rpt);
                                rpt.SetParameterValue(para, js.EvidenceInID.ToString());
