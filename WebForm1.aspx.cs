        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                try
                {
                    ...
                    {
                        var js = JsonConvert.DeserializeObject<InvestigateCodeJson>(requestFromPost);
                        if (js != null)
                        {
                            if (js.InvestigateCode != "")
                            {
                                ReportDocument rpt = new ReportDocument();
                                rpt.Load(Server.MapPath("~/ReportModel/" + filename + ".rpt"));
                                rpt = new cryslogin().Getsublogin(rpt);
                                rpt.SetParameterValue(para, js.InvestigateCode.ToString());
