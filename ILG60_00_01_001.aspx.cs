...
namespace Report_XCS
{
    ...
    public partial class ILG60_00_01_001 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                string filename = "ILG60-00-01-001";
                string para = "InvestigateDetailID";
                try
                {
                    ...
                    {
                        var js = JsonConvert.DeserializeObject<InvestigateDetailgetByConReq>(requestFromPost);
                        if (js != null)
                        {
                            if (js.InvestigateDetailID > 0)
                            {
                                ReportDocument rpt = new ReportDocument();
                                rpt.Load(Server.MapPath("~/ReportModel/" + filename + ".rpt"));
                                rpt = new cryslogin().Getsublogin(rpt);
                                rpt.SetParameterValue(para, js.InvestigateDetailID.ToString());
