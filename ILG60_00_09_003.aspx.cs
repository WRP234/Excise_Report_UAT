...
{
    public partial class ILG60_00_09_003 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                string filename = "ILG60-00-09-003";
                string para = "BribeRewardID";
                try
                {
                    ...
                    {
                        var js = JsonConvert.DeserializeObject<ILG60_00_09_003Json>(requestFromPost);
                        if (js != null)
                        {
                            if (js.BribeRewardID > 0)
                            {
                                ReportDocument rpt = new ReportDocument();
                                rpt.Load(Server.MapPath("~/ReportModel/" + filename + ".rpt"));
                                rpt = new cryslogin().Getsublogin(rpt);
                                rpt.SetParameterValue(para, js.BribeRewardID.ToString());
                            ...
                            else
                            {
                                ...
                                Response.Write("BribeRewardID : " + js.BribeRewardID.ToString() + "  ");
                                Response.End();
                            }
                        }
                    }
